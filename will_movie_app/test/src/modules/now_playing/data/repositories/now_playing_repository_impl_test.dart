...
import 'package:will_movie_app/src/modules/now_playing/data/adapters/now_playing_adapter.dart';
import 'package:will_movie_app/src/modules/now_playing/data/datasources/now_playing_datasource.dart';
import 'package:will_movie_app/src/modules/now_playing/data/repositories/now_playing_repository_impl.dart';
...
class NowPlayingDataSourceMock extends Mock implements NowPlayingDataSource {}

void main() {
  final datasource = NowPlayingDataSourceMock();
  final repository = NowPlayingRepositoryImpl(datasource);

  test('Get movies now playing', () async {
    when(() => datasource.getMovieNowPlaying())
        .thenAnswer((_) async => NowPlayingAdapter.fromMapList(moviesJson));
  ...
  test('Get NoDataFound', () async {
    when(() => datasource.getMovieNowPlaying())
        .thenAnswer((_) async => NowPlayingAdapter.fromMapList(moviesEmpty));
